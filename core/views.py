        ...
        async def state_callback(interaction: discord.Interaction):
            ...
            try:
                ...
                if state_image:
                    ...
                    # Edit the original message instead of creating new one
                    await self.original_interaction.edit_original_response(
                        content=f"🔍 **Close-up view of {state_name}**",
...
class MapMenuView(discord.ui.View):
    ...
    async def region_closeup(self, interaction: discord.Interaction):
        ...
        if region == "world":
            ...
            await interaction.response.edit_message(
                content="**🔍 Select a German state for close-up view:**", 
                view=view
            )
        else:
...
class MapPinButtonView(discord.ui.View):
    ...
    async def menu_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Get current map data to check proximity setting
        map_data = self.cog.maps.get(str(interaction.guild.id), {})
        allow_proximity = map_data.get('allow_proximity', False)
        
        # Create view with conditional button visibility
        view = MapMenuView(self.cog, int(interaction.guild.id))
        
        # Remove proximity button if disabled
        if not allow_proximity:
            # Find and remove the proximity button
            for item in view.children:
                if hasattr(item, 'emoji') and item.emoji and item.emoji.name == '📍':
