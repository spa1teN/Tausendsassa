...
class FeedListView(discord.ui.View):
    ...
    async def remove_feed(self, interaction: discord.Interaction, button: discord.ui.Button):
        feeds = await self.cog.get_guild_feeds(self.guild_id)
...
class FeedRemoveSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Remove the feed using async method
        success = await self.cog.remove_feed(self.guild_id, feed_name)
        ...
        if success:
            ...
            await interaction.response.edit_message(
                content=f"Failed to remove feed **{feed_name}**.",
                view=None
            )
...
class FeedConfigureView(discord.ui.View):
    ...
    def __init__(self, feeds: List[dict], cog, guild_id: int):
        ...
        if options:
            select = FeedConfigureSelect(options, cog, guild_id, feeds)
            self.add_item(select)


class FeedConfigureSelect(discord.ui.Select):
    """Select dropdown for feed configuration"""
    def __init__(self, options: List[discord.SelectOption], cog, guild_id: int, feeds: List[dict]):
        super().__init__(
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Find the feed config from the stored feeds
        feed_config = next((f for f in self.feeds if f.get("name") == feed_name), None)
...
class FeedConfigModal(discord.ui.Modal):
    ...
    def __init__(self, feed_config: dict, cog, guild_id: int, is_edit: bool = True):
        ...
        # Add crosspost field for editing only
        if is_edit:
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if color_hex is None:
            ...
            available = ', '.join(get_available_colors()[:10])
    ...
    async def _handle_edit_feed(self, interaction: discord.Interaction, color_hex: int, crosspost: bool):
        ...
        old_name = self.feed_config.get("name")

        # Create embed template based on feed type
        if is_bluesky_feed_url(self.url_input.value):
            embed_template = create_bluesky_embed_template(self.name_input.value, color_hex)
        else:
            embed_template = create_standard_embed_template(self.name_input.value, color_hex)
        ...
        success = await self.cog.update_feed(self.guild_id, old_name, updates)
        ...
        if success:
            ...
            await interaction.response.send_message(
                f"Failed to update feed **{self.name_input.value}**.", ephemeral=True
            )
...
class ChannelSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        success = await self.parent_view.cog.add_feed(self.parent_view.guild_id, new_feed)
        ...
        if success:
            ...
            await interaction.response.edit_message(
                content=f"Failed to add feed **{self.parent_view.name}**.",
                view=None
            )
